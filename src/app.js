...
import { loadModel, saveModel, resetModel, defaultModel } from './state.js';
...
function flagsChips(flags) {
  const chips = [];
  if (flags.revenueCappedByMargin) chips.push({ text: 'Revenue capped at 100% (Margin <80%)', kind: 'warn' });
...
function quarterRows(r) {
  return r.rollups.quarters.map(q => `
    <tr>
      <td>${q.key}</td>
...
function renderYOY(r) {
  ...
  const outcome = r.multipliers.yoyPayoutMult === 3 ? '300% (YOY met + Revenue ≥120%)'
                : r.multipliers.yoyPayoutMult === 2 ? '200% (YOY met + Revenue ≥100%)'
                : r.multipliers.yoyPayoutMult === 1 ? '100% (YOY met)'
                : '0% (YOY not met)';
...
function renderSettings(r) {
  ...
  const monthInputsRevenue = months.map((m, i) => `
    <div class="field">
  ...
  `).join('');
  const monthInputsMargin = months.map((m, i) => `
    <div class="field">
  ...
  el.innerHTML = `
    <div class="card">
      ...
      <div class="form">
        <div class="field">
          ...
          <input type="text" data-bind="employee_name" value="${model.employee_name||''}" />
        </div>
        <div class="field">
          <div class="label">Employee ID</div>
          <input type="text" data-bind="employee_id" value="${model.employee_id||''}" />
        </div>
        <div class="field">
    ...
    <div class="grid cols-2">
      ...
      <div class="card">
        ...
        <div class="form">
          ...
          <div class="field full"><div class="label">Guardrail notes (optional)</div><textarea data-bind="guardrails.notes">${model.guardrails.notes||''}</textarea></div>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="form">
        ...
        <div class="field full"><div class="label">Target revision notes</div><textarea data-bind="target_revision_notes">${model.target_revision_notes||''}</textarea></div>
      </div>
  ...
  // Save handler: read all data-bind inputs/areas
...
function setByPath(obj, path, raw) {
  ...
  if (last.match(/^\d+$/) && Array.isArray(cur)) {
    cur[Number(last)] = coerce(raw);
    return;
  }
  ...
  if (parts.length >= 3) {
    ...
    if (prev === 'monthly_revenue' || prev === 'monthly_margin') {
      // Actually here cur is model.actuals.monthly_revenue and last is index
      if (Array.isArray(cur)) {
        cur[Number(last)] = coerce(raw);
      } else {
        cur[last] = coerce(raw);
...
function coerce(v) {
  ...
  if (isFinite(n) && String(v).trim() !== '') {
    // preserve text inputs that are clearly not numeric
    // if original contains non-numeric chars (excluding . and -), keep string
    if (/[^0-9.\-]/.test(String(v))) return String(v);
    return n;
  }
  return v;
}
